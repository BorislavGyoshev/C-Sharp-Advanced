    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (input != "JOKER")
            {
                ...
                var cards = new HashSet<string>(inputParams.Skip(1));
        ...
        private static int CalcuateScore(HashSet<string> playerCards)
        {
            ...
            foreach (var card in playerCards)
            {
                ...
                if (!isNumber)
                {
                    switch (power)
                    {
                        case "J": powerInt = 11; break;
                        case "Q": powerInt = 12; break;
                        case "K": powerInt = 13; break;
                        case "A": powerInt = 14; break;
                    }
                }
                switch (type)
                {
                    case 'S': powerInt *= 4; break;
                    case 'H': powerInt *= 3; break;
                    case 'D': powerInt *= 2; break;
                    case 'C': powerInt *= 1; break;
                }
                totalScore += powerInt;
            }
            return totalScore;
